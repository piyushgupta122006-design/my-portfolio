...
export function setCharTimeline(
  ...
  character?.traverse((child: any) => {
    if (child.name === "Plane004" && child.children) {
    ...
    if (child.name === "screenlight" && child.material) {
      ...
      child.material.emissive?.set("#22d3ee");
      gsap.timeline({ repeat: -1, repeatRefresh: true }).to(child.material, {
  ...
  if (window.innerWidth > 1024 && character) {
    // Explicitly lock character at dead-center on page load / refresh
    gsap.set(".character-model", { x: 0, y: 0 });

    tl1
      .fromTo(character.rotation, { y: 0 }, { y: 0.7, duration: 1, immediateRender: false }, 0)
      .to(camera.position, { z: 22 }, 0)
      .fromTo(".character-model", { x: 0 }, { x: "-25%", duration: 1, immediateRender: false }, 0)
    ...
    tl2
      ...
      .to(".character-model", { pointerEvents: "none", x: "-18.5%", delay: 2, duration: 5 }, 0)
    ...
    tl3
      .to(".character-model", { y: "-100%", duration: 4, ease: "none", delay: 1 }, 0)
